          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              <>
                ...
                <Link
                  href="/dashboard"
                  className="flex items-center gap-2 text-sm text-slate-300 hover:text-white transition-colors bg-slate-900 border-2 border-black px-3 py-1.5 rounded-md shadow-[2px_2px_0px_#000] hover:-translate-y-0.5 active:translate-y-0 font-bold"
                >
        ...
        <div className="md:hidden bg-[#080a10] border-t-2 border-b-3 border-black">
          <div className="px-4 py-4 space-y-3">
            ...
            <div className="pt-2 border-t-2 border-black flex flex-col gap-2">
              {user ? (
                <>
                  <Link href="/dashboard" onClick={() => setIsOpen(false)} className="btn-secondary text-sm justify-center py-2">
                    <LayoutDashboard className="w-4 h-4" /> Dashboard
                  </Link>
                  <button onClick={() => { logOut(); setIsOpen(false); }} className="btn-secondary text-sm justify-center text-rose-500 py-2">
                    Sign Out
                  </button>
                </>
              ) : (
