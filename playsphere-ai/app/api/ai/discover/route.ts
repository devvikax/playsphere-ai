...
import { getAllVenues } from '@/lib/firebase/firestore';
...
Ensure variety in the insight types (e.g., one gap, one value, one trend).`;
    ...
    try {
      ...
      return NextResponse.json({ insights: getStaticFallbackInsights() });
    }

  } catch (error) {
    console.error('Discovery API Error:', error);
    return NextResponse.json({ insights: getStaticFallbackInsights() }, { status: 500 });
  }
}

function getStaticFallbackInsights() {
  return [
    {
      type: 'value',
