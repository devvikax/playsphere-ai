...
class Command(BaseCommand):
    ...
    def safe_write(self, msg: str, style_func=None):
        try:
            ...
            # Remove any remaining unencodable characters
            clean_msg = "".join(c for c in safe_msg if ord(c) < 128)
            self.stdout.write(style_func(clean_msg) if style_func else clean_msg)

    def handle(self, *args, **options):
        self.safe_write("Starting PS-25 Alignment Verification Suite (Python)...", self.style.NOTICE)
        result = asyncio.run(self._run_suite())

        if result["success"]:
            self.safe_write("\n✅ All tests PASSED!", self.style.SUCCESS)
        else:
            self.safe_write(f"\n❌ Test suite FAILED: {result.get('error')}", self.style.ERROR)
    ...
    async def _run_suite(self) -> dict:
        ...
        logs: list[str] = []
        write_func = self.safe_write

        def log(msg: str):
            write_func(f"  [TEST] {msg}")
            logs.append(msg)
